...
test.describe("wire checkout + admin confirm", () => {
  ...
  test("wire commitment moves to awaiting_wire and admin can mark it funded", async ({
    ...
    await adminPage
      ...
      .selectOption("awaiting_wire");
    adminPage.once("dialog", (d) => d.accept());
    await Promise.all([
      adminPage.waitForResponse(
        (r) =>
          r.url().includes(`/api/admin/commitments/${c.id}/confirm-wire`) &&
          r.request().method() === "POST",
      ),
      adminPage.getByTestId(`button-confirm-wire-${c.id}`).click(),
    ]);

    // Verify commitment is now funded via the legacy admin API.
